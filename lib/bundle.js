...
/***/ function(module, exports, __webpack_require__) {
	...
	class Game {
	  ...
	  step() {
	    console.log(this.isOscillating10());
	    this.changeSquares();
	  ...
	  findLivePositions() {
	    return Object.keys(this.squares).filter((position) => {
	      return this.squares[[position]].live;
	    }).map(position => [parseInt(position.split(",")[0]), parseInt(position.split(",")[1])]);
	  }
