...
class Game {
  ...
  objectAttackClick(e) {
    ...
    this.squares = {};
    this.addSquares();
    this.objectAttack(e.currentTarget.innerHTML);
    this.addLiveCount();
  }

  // gliderAttackButton() {
  //   const gliderAttack = document.getElementById("glider-attack");
  //   gliderAttack.addEventListener("click", this.gliderAttack.bind(this));
  // }
  //
  // gliderAttack() {
  //   Game.OBJECTATTACK = true;
  ...
  objectPlaceButtons() {
    const objectPlaceButtons = document.getElementsByClassName('object-place');
    for (let i = 0; i < objectPlaceButtons.length; i++) {
      objectPlaceButtons[i].addEventListener("click", this.objectPlaceClick.bind(this));
    }
  }
  ...
  objectPlaceClick(e) {
    ...
    this.squares = {};
    this.addSquares();
    const count = parseInt(document.getElementById("object-place-count-input").value);
    const objectName = document.getElementById("object-place-input").value;
    this.placeObjects(objectName, count);
    this.addLiveCount();
  }
  // if (Game.RANDOMOBJECTS) {
  //   this.placeRandomObjectsRandomly(Game.RANDOMOBJECTSCOUNT);
